            ...
            if self.adapter_type in ['i2v']:
                ...
                if self.config.has_inpainting_input:
                    ...
                    if inpaint_tensor is not None and not do_dropout:
                        ...
                        if inpaint_tensor.shape[1] == 4:
                            # get just the mask
                            inpainting_tensor_mask = inpaint_tensor[:, 3:4, :, :].to(latents.device,
                                                                                     dtype=latents.dtype)
                        elif inpaint_tensor.shape[1] == 3:
                            # rgb mask. Just get one channel
                            inpainting_tensor_mask = inpaint_tensor[:, 0:1, :, :].to(latents.device,
                                                                                     dtype=latents.dtype)
                        else:
                        ...
                        inpainting_tensor_mask = F.interpolate(inpainting_tensor_mask, size=(inpainting_latent.shape[2],
                                                                                             inpainting_latent.shape[
                                                                                                 3]), mode='bilinear')
                        ...
                        inpainting_latent = torch.cat(
                            (inpainting_latent, torch.ones_like(inpainting_latent[:, :1, :, :])), dim=1)
                ...
                for control_tensor in control_tensor_list:
                    ...
                    if do_dropout:
                        ...
                        if control_tensor.shape[2] != batch.tensor.shape[2] or control_tensor.shape[3] != \
                                batch.tensor.shape[3]:
                            control_tensor = F.interpolate(control_tensor,
